...
function authenticateMFUsingUserStore() {
    ...
    var authParams = {
        "userid": mfConfig.identityServices[0].username,
        "password": mfConfig.identityServices[0].password
    };
...
function getProductCategories() {
    ...
    if (kony.net.isNetworkAvailable(constants.NETWORK_TYPE_ANY)) {
        kony.application.showLoadingScreen("loadskin", "Fetching Categories !!!", constants.LOADING_SCREEN_POSITION_FULL_SCREEN, false, true, {
            enableMenuKey: true,
            enableBackKey: true,
            progressIndicatorColor: "ffffff77"
        });
    ...
    if (CategoriesResponse !== null && CategoriesResponse.opstatus === 0) {
        ...
        if (CategoriesResponse.Categories !== null) {
            ...
            if (CategoriesResponse.Categories.length > 0) {
                ...
                frmHome.segForDataHomeScreen.widgetDataMap = {
                    lblForDataHomeScreen: "Name",
                    ID: "CategoryID"
                };
        ...
        kony.ui.Alert({
            message: "Service call failed with opstatus " + CategoriesResponse.opstatus,
...
function getSubCatSuccessCallback(SubCategoriesResponse) {
    ...
    if (SubCategoriesResponse !== null && SubCategoriesResponse.opstatus === 0) {
        ...
        // The call failed because opstatus was not 0 so we'll alert the user and show that opststus
        kony.ui.Alert({
            message: "Service call failed with opstatus " + SubCategoriesResponse.opstatus,
            alertType: constants.ALERT_TYPE_ERROR,
            alertTitle: "Fox News",
            yesLabel: "OK"
        }, {});
...
function getProductDetails() {
    ...
    if (selectedKey == "null" || selectedKey == undefined || selectedKey == "") {
        ...
        if (kony.net.isNetworkAvailable(constants.NETWORK_TYPE_ANY)) {
            kony.application.showLoadingScreen("loadskin", "Fetching Product Details !!!", constants.LOADING_SCREEN_POSITION_FULL_SCREEN, false, true, {
                enableMenuKey: true,
                enableBackKey: true,
                progressIndicatorColor: "ffffff77"
            });
...
function getProductDetailsSuccessCallback(ProductDetailsResponse) {
    ...
    if (ProductDetailsResponse !== null && ProductDetailsResponse.opstatus === 0) {
        ...
        // The call failed because opstatus was not 0 so we'll alert the user and show that opststus
        kony.ui.Alert({
            message: "Service call failed with opstatus " + ProductDetailsResponse.opstatus,
            alertType: constants.ALERT_TYPE_ERROR,
            alertTitle: "Fox News",
            yesLabel: "OK"
        }, {});
...
function getProductReviews() {
    ...
    // }
    if (kony.net.isNetworkAvailable(constants.NETWORK_TYPE_ANY)) {
        kony.application.showLoadingScreen("loadskin", "Fetching Reviews !!!", constants.LOADING_SCREEN_POSITION_FULL_SCREEN, false, true, {
            enableMenuKey: true,
            enableBackKey: true,
            progressIndicatorColor: "ffffff77"
        });
...
function getProductReviewsSuccessCallback(ProductReviewsResponse) {
    ...
    if (ProductReviewsResponse !== null && ProductReviewsResponse.opstatus === 0) {
        ...
        if (ProductReviewsResponse.ReviewCollection !== null) {
            ...
            if (ProductReviewsResponse.ReviewCollection.length > 0) {
                ...
                for (i = 0; i < productdetails.length; i++) {
                    ...
                    var record = {
                        "ReviewID": productdetails[i].ReviewID,
        ...
        // The call failed because opstatus was not 0 so we'll alert the user and show that opststus
        kony.ui.Alert({
            message: "Service call failed with opstatus " + ProductReviewsResponse.opstatus,
            alertType: constants.ALERT_TYPE_ERROR,
            alertTitle: "Fox News",
            yesLabel: "OK"
        }, {});
